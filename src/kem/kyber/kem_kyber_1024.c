...
int PQCLEAN_KYBER1024_AVX2_crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk);
int PQCLEAN_KYBER1024_AVX2_crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk);
#endif


OQS_API OQS_STATUS OQS_KEM_kyber_1024_keypair(uint8_t *public_key, uint8_t *secret_key) {
#if defined(OQS_USE_AES_INSTRUCTIONS)  && defined(OQS_USE_AVX2_INSTRUCTIONS)  && defined(OQS_USE_BMI2_INSTRUCTIONS)  && defined(OQS_USE_POPCNT_INSTRUCTIONS)  && defined(OQS_ENABLE_KEM_kyber_1024_avx2)
